...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, attn_args: AttnArgs):
        ...
        # Paired-head attention doubles effective sequence length; match the original positional lookback by doubling the window.
        paired_bm_size = None if bm_size is None else (2 * bm_size)
        y = flash_attn_interface.flash_attn_varlen_func(q[0], k[0], v[0], cu_seqlens_q=seqlens, cu_seqlens_k=seqlens,
                                                        max_seqlen_q=max_len, max_seqlen_k=max_len,
                                                        causal=True, softmax_scale=yarn.attn_scale, window_size=(paired_bm_size, 0))
...
class TrainingManager():
    ...
    def advance_schedule(self, step: int):
        ...
        if new_ws_long != self.ws_long:
            ...
            # Paired-head attention runs at 2x sequence length; keep YaRN scaling consistent by using doubled windows.
            self.model.yarn_paired_head.apply(2 * self.ws_long, 2 * new_ws_long)
